...
DESCRIPTION="Vertcoin GPU miner in C"
HOMEPAGE="http://vertcoin.org/ https://github.com/Bufius/vertminer-gpu"
EGIT_REPO_URI="https://github.com/Bufius/vertminer-gpu.git"
...
src_configure() {
	...
	sed -i 's~^\(\#define CGMINER_PREFIX \).*$~\1"'"${EPREFIX}/usr/lib/vertminer"'"~' config.h
}
...
src_install() { # How about using some make install?
	...
	if use modminer; then
		insinto /usr/lib/vertminer/modminer
		doins bitstreams/*.ncd
		dodoc bitstreams/COPYING_fpgaminer
	fi
	if use opencl; then
		insinto /usr/lib/vertminer
		doins *.cl
	fi
